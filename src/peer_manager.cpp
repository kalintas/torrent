...
void PeerManager::calculate_handshake(std::string_view info_hash, std::string_view peer_id)  {
    if (info_hash.size() != 20 || peer_id.size() != 20) {
        throw new 
            std::runtime_error("Error while calculating the peer handshake. Illegal arguments");
    } 

    static constexpr std::string_view protocol_identifier = "BitTorrent protocol";
    handshake[0] = 19;
    std::memcpy(handshake.data() + 1, protocol_identifier.data(), protocol_identifier.size());
...
void PeerManager::remove(const tcp::endpoint& endpoint) {
    ...
    BOOST_LOG_TRIVIAL(info) << "Peer count: " << peers.size() 
