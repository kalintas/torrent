...
void Peer::change_status(Status new_status) {
    ...
    switch (status) {
        case Status::Connected:
            BOOST_LOG_TRIVIAL(info) << "Peer count: " << peer_manager.peer_count() 
    ...
    [this, ptr] (const auto& error, const auto bytes_read) {
        if (error) {
            ...
            if (status == Status::Connected) {
                ...
                bool is_header_equal = std::equal(buffer.begin(), buffer.begin() + 20, 
                           our_handshake.begin(), our_handshake.begin() + 20); 
               
                bool is_info_hash_equal = std::equal(buffer.begin() + 28, buffer.begin() + 48, 
                        our_handshake.begin() + 28, our_handshake.begin() + 48);                 
                ...
                std::memcpy(remote_peer_id.data(), buffer.data() + 48, remote_peer_id.size());
                change_status(Status::Handshook); 
            }

            BOOST_LOG_TRIVIAL(info) << "Read " << bytes_read 
                << " bytes from peer: " << *this; 
            listen_peer();
        } 
    });
}
...
void Peer::start_handshake() {
    auto ptr = shared_from_this();
    socket.async_send(asio::buffer(peer_manager.get_handshake()), 
